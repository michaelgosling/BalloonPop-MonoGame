...
namespace BalloonPop.GameObjects
{
	...
	class Balloon
	{
		...
		{
			if (!MatchedIDs.Contains(id)) MatchedIDs.Add(id);
		}

		/// <summary>
		/// Provides a new ID from a static counter
		/// </summary>
		/// <returns>Integer representing IDs</returns>
		...
		public void DetectStageCollision()
		{
			...
			if (this.BoundingBox.Right >= Constants.PLAY_AREA.Right || this.BoundingBox.Left <= Constants.PLAY_AREA.Left)
			{
				this.velocity.X *= -1;
			}
